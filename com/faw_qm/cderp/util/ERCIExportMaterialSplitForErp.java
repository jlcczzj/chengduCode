...
import com.faw_qm.cderp.ejb.service.MaterialSplitServiceEJB;
import com.faw_qm.cderp.exception.ERPException;
import com.faw_qm.cderp.model.FilterPartInfo;
import com.faw_qm.cderp.model.MaterialPartStructureInfo;
 ...
 * @version 1.0
 */
public class ERCIExportMaterialSplitForErp
{
	...
	public void mainExport() throws QMException {
		...
		result = select.executeQuery("select * from MESData");
...
//			制造路线
			 FilterPartInfo filterPart = new FilterPartInfo();
	          ...
	          filterPart.setRoute(routeAllCode);
...
//				这里为什么直接就是N呢？questions
				...
				if(usageLinkList!=null&&usageLinkList.size()>0)
				{
					...
					while(linkIte.hasNext())
					{
						...
						mPartStructureIfc.setParentPartNumber(mSplitIfc.getMaterialNumber());
						mPartStructureIfc.setParentPartVersion(mSplitIfc.getPartVersion());
						mPartStructureIfc.setParentNumber(partIfc.getPartNumber());
...
//				需要进行物料拆分的情况下，首先将制造路线分解为一个个路线字符
				...
				for(int m = routeCodeList.size() - 1; m >= 0; m--)
				{
					...
					String makeCodeNameStr = RemoteProperty.getProperty("com.faw_qm.cderp.routecode." + routeCode);
...
//						routeCodeMap.put(makeCodeNameStr, "0");
					...
					for(int k = m-1; k >= 0; k--){
						...
						String makeCodeNameStr1 = RemoteProperty.getProperty("com.faw_qm.cderp.routecode." + routeCode1);
...
//					}
				...
				for(int ii=0;ii<objs.length;ii++)
			    {
				     ...
				     if(ii!=objs.length-1)
				     {
				      ...
				      String k1 = RemoteProperty.getProperty("com.faw_qm.cderp.routecode." + ifc.getRouteCode());
				      String k2 = RemoteProperty.getProperty("com.faw_qm.cderp.routecode." + ifc1.getRouteCode());
				      ...
				      if(k1!=null&&k2!=null){
				    	...
				    	//CCBegin by chudaming 20100322,如果k1和k2都不为空，并且k1和k2都不等于SC(生产部的不按相同车间一样处理)
				    	  ...
				    	  if((ifc.getMaterialNumber().equals(partIfc.getPartNumber()))&&
				    			  ...
				    			  !k1.trim().equalsIgnoreCase("S")&&!k2.trim().equalsIgnoreCase("S)")){
...
//		    			  )&&!k1.trim().equalsIgnoreCase("SC")&&!k2.trim().equalsIgnoreCase("SC)")){
				    	//CCBegin by dikefeng ,20100624，将前顺序进行了调整，有可能物料编码的长度根本就不到2为，就会出现字符串越界错误
				    	  else if(k1.toString().equals(k2.toString())&&!k1.trim().equalsIgnoreCase("S")&&!k2.trim().equalsIgnoreCase("S)")&&
...
//		}
    	...
    	String makeCodeNameStr1 = RemoteProperty.getProperty("com.faw_qm.cderp.routecode." + ms.getRCode());
...
//    		System.out.println("asasasasas=========0628======="+as);
    		for(int x =0;x<as.size()-1;x++){
    			...
    			String x11 = RemoteProperty.getProperty("com.faw_qm.cderp.routecode." + x1);
    			String x22 = RemoteProperty.getProperty("com.faw_qm.cderp.routecode." + x2);
