...
public class QMXMLMaterialStructure extends QMXMLData
{
    ...
    private final String getDefaultUnit()
    {
        return materialStructure.getDefaultUnit() == null ? ""
                : materialStructure.getDefaultUnit();
    }
...
//刘家坤解放 2014
    ...
    {
        return materialStructure.getSubGroup() == null ? ""
                : materialStructure.getSubGroup();
    }
    /**
     * bom行向虚拟。
     * @return bom行向虚拟。
     */
    public final String getBOMLine()
    {
        return materialStructure.getBomLine() == null ? ""
                : materialStructure.getBomLine();
    }//CCBegin SS1
    /**
     * 替换前零件号
     * @return 子组号。
     */
    public final String getBeforeMaterial()
    {
        return materialStructure.getBeforeMaterial() == null ? ""
                : materialStructure.getBeforeMaterial();
    }
    /**
     * 替换前数量。
     * @return 替换前数量。
     */
    public final String getBeforeQuantityStr()
    {
    	 String quantity=Float.toString(materialStructure.getBeforeQuantity());
...
//            System.out.println("getBeforeQuantityStr()()=="+getBeforeQuantityStr());
            ...
            else if(property.getName().equals("subGroup"))
            {
            	
                col.setId(property.getId());
                col.setValue(getSubGroup());
            }
            else if(property.getName().equals("BOMLine"))
            {
                col.setId(property.getId());
                col.setValue(getBOMLine());
            }//CCBegin SS1
            else if(property.getName().equals("beforeQuantity"))
            {
            	System.out.println("88888888888888888");
                col.setId(property.getId());
                col.setValue(getBeforeQuantityStr());
            }
            else if(property.getName().equals("beforeMaterial"))
            {
                col.setId(property.getId());
                col.setValue(getBeforeMaterial());
            }//CCEnd SS1
            colList.add(col);
        }
