...
import com.faw_qm.cderp.exception.IntePackException;
import com.faw_qm.cderp.model.IntePackIfc;
import com.faw_qm.cderp.model.IntePackInfo;
import com.faw_qm.cderp.util.BaseDataPublisher;
import com.faw_qm.cderp.util.IntePackSourceType;
import com.faw_qm.cderp.util.RequestHelper;
import com.faw_qm.cderp.util.RouteCodeIBAName;
import com.faw_qm.cderp.util.TechnicsDataPublisher;
...
//        }
     ...
     * @throws QMExcepiton
     */
    public IntePackInfo publishIntePack(String id ,String xmlName,Collection coll,Vector filterParts ,Collection vec,int i)throws QMException
    {
    	try{
    	BaseDataPublisher.setXmlName(xmlName);
    	BaseDataPublisher.setColleciton(coll);
    	...
    	BaseDataPublisher.setBom(vec);//添加单级bom结构关系
    	BaseDataPublisher.setxmlid(i);//如果存在一次发布多个xml文件，批量更改功能。此处设置流水
    	//下面方法中有调用发布数据方法publish（）；
        IntePackInfo info = publishIntePack(id);
        //RequestHelper.initRouteHashMap();
        return info;
    	}catch(QMException e){
...
//        如果当前用户没有修改集成包的权限
        ...
        {
            //首先发布数据，此处调用发布数据方法
        	System.out.println("intePackInfo="+intePackInfo);
        	System.out.println("intePackInfo.getSource()="+intePackInfo.getSource());
        	String source = intePackInfo.getSource();
        	if(source==null||source.length()==0){
        		BaseDataPublisher.publish(intePackInfo);
        	}else{
