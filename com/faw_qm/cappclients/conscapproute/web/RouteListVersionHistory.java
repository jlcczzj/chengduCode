 ...
 * 可在本公司授权范围内，使用本程序
 * 保留所有权利
 * CR1 2011/12/14 徐春英     原因：Version专题更改    参见Version专题更改说明1_张强.doc
 */
...
/**
 * （问题一）zz 周茁修改 查看历史版本时不应该看见检出的路线表 <p>Title:查看路线表的版本历史 </p> <p>Description: 本类提供的方法为廋客户界面使用</p> <p>Copyright: Copyright (c) 2006</p> <p>Company: 一汽启明</p>
 * @author 刘明
 * @version 1.0
 */
public class RouteListVersionHistory
{
    ...
    public static String getVersionHead(String BsoID)
    {
        ...
        if(obj != null)
        {
            ...
            {
                technicsNumber = ((TechnicsRouteListIfc)obj).getRouteListNumber();
                technicsName = ((TechnicsRouteListIfc)obj).getRouteListName();
                rStr = "路线表编号：" + technicsNumber + " ---------" + " 路线表名称：" + technicsName;
                return rStr;
            }else
            {
                technicsNumber = ((TechnicsRouteListMasterIfc)obj).getRouteListNumber();
                technicsName = ((TechnicsRouteListMasterIfc)obj).getRouteListName();
                rStr = "路线表编号：" + technicsNumber + " ---------" + " 路线表名称：" + technicsName;
                return rStr;
            }
        }else
    ...
    public static Collection getVersionsCollection(String BsoID)
    {
        ...
        if(obj != null)
        {
            try
            {
                VersionControlService service1 = (VersionControlService)EJBServiceHelper.getService("VersionControlService");
                ConfigService configService = (ConfigService)EJBServiceHelper.getService("ConfigService");//CR1
                if(obj instanceof TechnicsRouteListIfc)

                    myVersionCollection = (Collection)(service1.allVersionsOf((TechnicsRouteListIfc)obj));
                else
                    myVersionCollection = (Collection)(configService.filteredIterationsOf((MasteredIfc)obj));//CR1
            }catch(Exception e)
            {
                e.printStackTrace();
            }
        ...
        for(int i = 0;i < versionCollection.size();i++)
        {
            ...
            try
            {
                ...
                if(((LifeCycleManagedIfc)technics).getProjectId() != null)
                    project = (ProjectIfc)pService.refreshInfo(((LifeCycleManagedIfc)technics).getProjectId());
            ...
            if(technics.getIterationCreator() != null)
            {
                try
                {
                    objectCreator = RouteWebHelper.getUserNameByID((technics.getIterationCreator()));
                }catch(Exception e)
                {
                    objectCreator = "";
                }
            ...
            //构造数组
            String[] myVersionArray1 = {versionBsoID, versionID, objectState, projectName, objectCreator, createTime};
            //加到集合中
