...
import com.faw_qm.cappclients.beans.cappexattrpanel.CappExAttrPanelDelegate;
...
public class CappExAttrPanelForZC extends CappExAttrPanel{
	 ...
	 //资源文件
	   ...
	   */
	  public GroupPanelForZC groupPanel;

	  /**
	   * 控制计划面板
	   */
	  GroupPanelForZC controlPlanPanel;
	 ...
	 // 主动属性位置
	   ...
	   */
	  public CappExAttrPanelForZC(String bsoName,String secondName,int model) throws QMException
	  {
	    try {
	  ...
	  public CappExAttrPanelForZC(String bsoName,String secondName,int model,ArrayList vec)throws QMException
	  {
		  try {
			  this.editVec = vec;
		      this.model =model;
...
//	      jScrollPane1.getViewport().add(attrPanel, null);
	    ...
	    if(isGroup)
	    {
	      ...
	      while(names.hasNext())
	      {
	        ...
	        groupPanel = (GroupPanelForZC)this.groupMaps.get(name);
	  ...
	  private void prepareValues(){
	    ...
	    if(isGroup)
	    {
	      ...
	      {
	        name = (String)names.next();
	        groupPanel = new GroupPanelForZC(container,name,false);
...
//	        groupPanel.defaultColumnWidths();1111111111111111111111111111111111
	    ...
	    if(this.isControlPlan)
	    {
	      this.controlPlanPanel = new GroupPanelForZC(container,"ControlPlan",true);
	       ...
	       // 20070205薛静修改
	        ...
	        public void show(ExtendAttContainer container1)
	        {
	            ...
	            if (isGroup)
	            {
	                Iterator names = container.getAttGroupNames().iterator();
	                String name;
	                GroupPanelForZC gpanel = null;
	                while (names.hasNext())
	                {
	                    name = (String) names.next();
	                    gpanel = (GroupPanelForZC)this.groupMaps.get(name);
...
//	      this.controlPlanPanel.show(info,"ControlPane");
	  ...
	  public  boolean check()
	  {
	    ...
	    if(this.isGroup)
	   {
	    	Iterator names = container.getAttGroupNames().iterator();
	    	String name;
	    	GroupPanelForZC gpanel ;
	    	while(names.hasNext()){
	    	name = (String)names.next();
	    	gpanel = (GroupPanelForZC)groupMaps.get(name);
	    if(!gpanel.check())
	      return false;
	   }
	   }
	  ...
	  public ExtendAttContainer getExAttr()
	  {
	    ...
	    if(this.isGroup)
	    {
	    	...
	    	while(names.hasNext()){
	    	name = (String)names.next();
	    	gp = (GroupPanelForZC)this.groupMaps.get(name);
