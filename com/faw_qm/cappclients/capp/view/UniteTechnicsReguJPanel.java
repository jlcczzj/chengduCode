/** 生成程序UniteTechnicsReguJPanel.java	1.1  2003/08/15
 * 版权归一汽启明公司所有
 * 本程序属一汽启明公司的私有机要资料
 * 未经本公司授权，不得非法传播和盗用
 * 可在本公司授权范围内，使用本程序
 * 保留所有权利
 * CR1 2009/04/28 李磊   原因：制件流程图的修改，简图字段分开为两个
 *                       方案：将简图字段分开存储 
 * CR2 2009/12/21 徐春英 原因：参见测试域v4r3FunctionTest,DefectID=2687
 * CR3 刘志城  20091221  原因：参见DefectID=2776 
 * CR4 2010/01/28 徐春英 原因: 参见td问题2859
 * SS1 判断所属公司，变速箱公司不显示 TS16949属性面板 说liuyang 2013-3-27
 * SS2 变速箱公司备注显示为PFEMA编号 liuyang 2013-3-27
 * SS3 只显示代码分类 liuyang 2013-3-27
 */                         
...
/**
 * <p>Title:合并工艺规程界面 </p>
 * <p>Description:本用例用于需要多人编辑同一工艺的情况。用例执行者创建新的工艺主信息，</p>
 * <p>并将其它用户创建完成的工艺合并到该工艺中形成新的完整的工艺。 </p>
 * <p>合并操作使工艺列表中的工艺主信息数据及版本丢失，取而代之的是新创建工艺的主信息,</p>
 * <p>增加原工序的最新版本与新工艺主信息版本关联。</p>
...
//        workShopSortingSelectedPanel.setIsSelectCC(true);
        ...
        //问题（1）2008.03.28 刘志城修改 修改原因：工艺规程合并主信息资料夹项可编辑时报两遍提示信息。
        //                              应改为：不可编辑，报一次提示信息。
...
//      CCBegin by leix 2010-3-18 原因：根据解放要求，工艺主信息产品状态默认为生产。
        productStateComboBox.setSelectedIndex(2);
//      CCEnd by by leix 2010-3-18 原因：根据解放要求，工艺主信息产品状态默认为生产。
        //CCBegin SS1
        if(!BSXGroup()){
          if(ts16949){  
...
//        lifeCyclePanel.setButtonName("搜索");
...
//        lifeCyclePanel.setObject((LifeCycleManagedIfc)technicsInfo);
    ...
    private void newCappExAttrPanel(String technicsType)
    {
        ...
        try
        {
            ...
            //CCBegin by liuzhicheng 2010-01-30 原因：合并工艺卡其他关联信息面板查看时滚动条不起作用。
//          if (extendTable.get(technicsType) == null)
//          {
          //CCEnd by liuzhicheng 2010-01-30 原因：合并工艺卡其他关联信息面板查看时滚动条不起作用。
