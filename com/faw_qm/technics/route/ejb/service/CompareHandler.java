...
public class CompareHandler {
  ...
  protected static Map compareIterate(Collection listVec) throws QMException {
    ...
    if (listInfo.length < 2) {
      throw new TechnicsRouteException("给定集合至少包含两个元素。");
    }
    ...
    if (listInfo.length == 2) {
      ...
      for (Iterator iter = addVec.iterator(); iter.hasNext(); ) {
        ...
        hashmap.put(linkInfo.getPartMasterID() + ";" +
                    linkInfo.getParentPartNum(), linkVec);
      }
      for (Iterator iter = updateMap.keySet().iterator(); iter.hasNext(); ) {
        ListRoutePartLinkIfc linkInfo1 = (ListRoutePartLinkIfc) iter.next();
        ListRoutePartLinkIfc linkInfo2 = (ListRoutePartLinkIfc) updateMap.get(
            linkInfo1);
        Collection linkVec = new Vector();
        linkVec.add(linkInfo1);
        linkVec.add(linkInfo2);
        hashmap.put(linkInfo1.getPartMasterID() + ";" +
                    linkInfo1.getParentPartNum(), linkVec);
      }
   ...
   */
  private static void handleIterateResult(Collection allPartVec,
                                          TechnicsRouteListIfc[] listInfo,
                                          Map hashmap) throws QMException {
    if (TechnicsRouteServiceEJB.VERBOSE) {
      System.out.println(
          "enter the class:CompareHandler,method:handleIterateResult:119行");
    ...
    for (Iterator iter = allPartVec.iterator(); iter.hasNext(); ) {
      ...
      Collection linkVec = new Vector();
      for (int i = 0; i < listInfo.length; i++) {
        ListRoutePartLinkIfc linkInfo = RouteHelper.getRouteService().
            getListRoutePartLink(listInfo[i].getBsoID(), partMasterID,
                                 partNumber);
        linkVec.add(linkInfo);
      }
   ...
   * @throws QMException
   * @return Collection  partVec包含:addVec:增加的零件集合,updateMap:更改零件的
   * 路线（包括删除的路线） updateMap: key : A.1对应的更改关联，value : A.4对应的更改关联;
   * delVec:删除的零件Collection。
   */
  private static Collection getPartMaster(TechnicsRouteListIfc iterated1,
                                          TechnicsRouteListIfc iterated2) throws
      QMException {
   ...
   */
  private static Object[] compareIterate(TechnicsRouteListIfc iterated1,
                                         TechnicsRouteListIfc iterated2) throws
      QMException {
    if (TechnicsRouteServiceEJB.VERBOSE) {
      System.out.println(
          "enter routeService's comparteIterate iterated1.bsoid = " +
          iterated1.getBsoID() +
          " iterated2.bsoid = " + iterated2.getBsoID());
    }
    ...
    if (iterateID2.compareTo(iterateID1) < 1) {
      if (iterateID2.equals(iterateID1)) {
        if (! (iterated1.getWorkableState().equals(CHECKOUT))) {
          throw new TechnicsRouteException("给定版本的顺序不对：iterated1的版序： " +
                                           iterateID1 + ", iterated2的版序： " +
                                           iterateID2);
        }
      }
      else {
    ...
    //获得iterated1的所有关联。
    Collection coll1 = RouteHelper.getRouteService().getRouteListLinkParts(
        iterated1);
    if (TechnicsRouteServiceEJB.VERBOSE) {
      System.out.println(TechnicsRouteServiceEJB.VERBOSE +
                         "iterated1的所有关联 : coll1 =" + coll1);
      //获得iterated1到iterated2之间版本增加、更改和删除的路线。
    }
    Collection coll2 = getCreateLink(iterated1, iterated2);
    if (TechnicsRouteServiceEJB.VERBOSE) {
      System.out.println(TechnicsRouteServiceEJB.VERBOSE +
                         " iterated1到iterated2之间版本增加、更改和删除的路线。coll2 = " + coll2);
      //获得新增的零件，但尚未创建路线。此时获得的关联有可能是从以前版本继承过来的，需要过滤。
    }
    Collection coll2_1 = getNewLink(iterated2);
    if (TechnicsRouteServiceEJB.VERBOSE) {
      System.out.println(TechnicsRouteServiceEJB.VERBOSE +
                         "新增的零件，但尚未创建路线。coll2_1 = " + coll2_1);
      //设置增加和更改（包括删除）路线的数组。
    }
    filterCreateAndUpdateLink(coll1, coll2, coll2_1, obj);
    if (TechnicsRouteServiceEJB.VERBOSE) {
      System.out.println(TechnicsRouteServiceEJB.VERBOSE + "所有的新增零件: coll2 = " +
                         coll2);
      //获得删除的关联。有可能重新建立关联。
    }
    ...
    if (TechnicsRouteServiceEJB.VERBOSE) {
      System.out.println(TechnicsRouteServiceEJB.VERBOSE +
                         "获得删除的关联，有可能重新建立关联: coll3 = " + coll3);
      //对关联进行过滤。
    }
    filterDeleteLink(coll3, coll2);
    if (TechnicsRouteServiceEJB.VERBOSE) {
      System.out.println(TechnicsRouteServiceEJB.VERBOSE +
                         "删除的关联去掉重新建立的关联: coll3 = " + coll3);
    }
    obj[2] = coll3;
    if (TechnicsRouteServiceEJB.VERBOSE) {
      System.out.println(TechnicsRouteServiceEJB.VERBOSE +
                         "exit routeService's comparteIterate obj[0] = " +
                         obj[0] +
                         ", obj[1].key = " +
                         ( (Map) obj[1]).keySet() + "obj[1].values = " +
  ...
  private static void filterDeleteLink(Collection coll1, Collection coll2) {
    for (Iterator iter = coll2.iterator(); iter.hasNext(); ) {
      ...
      for (Iterator iter1 = coll1.iterator(); iter1.hasNext(); ) {
        ...
        if (numEql &&
            listLinkInfo1.getPartMasterID().trim().equals(listLinkInfo.
            getPartMasterID().trim())) {
   ...
   * @param obj Object[]
   */
  private static void filterCreateAndUpdateLink(Collection coll1,
                                                Collection coll2,
                                                Collection coll3, Object[] obj) {
    ////////////对尚未创建路线的情况作单独处理。///////////////////////////////////////
    //coll3在coll2中过滤。
    ...
    if (TechnicsRouteServiceEJB.VERBOSE) {
      System.out.println(TechnicsRouteServiceEJB.VERBOSE +
                         "新增的零件，但尚未创建路线。第一次过滤：coll3 = " + coll3);
    ...
    if (TechnicsRouteServiceEJB.VERBOSE) {
      System.out.println(TechnicsRouteServiceEJB.VERBOSE +
                         "新增的零件，但尚未创建路线的关联的最后结果：coll3 = " + coll3);
      /////////获得路线有变化，添加或更改的关联。///////////////////////////////////////////////
    }
    Map hashmap = new TreeMap();
    for (Iterator iter1 = coll1.iterator(); iter1.hasNext(); ) {
   ...
   * @return Collection
   */
  private static Collection getCreateLink(TechnicsRouteListIfc iterated1,
                                          TechnicsRouteListIfc iterated2) throws
      QMException {
  ...
  private static Collection getNewLink(TechnicsRouteListIfc iterated2) throws
      QMException {
    ...
    QueryCondition cond1 = new QueryCondition("alterStatus",
                                              ...
                                              TechnicsRouteServiceEJB.INHERIT);
    query.addCondition(cond1);
    query.addAND();
    QueryCondition cond2 = new QueryCondition(TechnicsRouteServiceEJB.LEFTID,
                                              QueryCondition.EQUAL,
                                              iterated2.getBsoID());
    ...
    QueryCondition cond4 = new QueryCondition("adoptStatus",
                                              QueryCondition.EQUAL,
                                              RouteAdoptedType.CANCEL.toString());
  ...
  private static Collection getSelfAlter(TechnicsRouteListIfc iterated2) throws
      QMException {
    PersistService pservice = (PersistService) EJBServiceHelper.
        getPersistService();
    QMQuery query = new QMQuery(TechnicsRouteServiceEJB.
    ...
    QueryCondition cond = new QueryCondition("alterStatus",
                                             QueryCondition.EQUAL,
                                             TechnicsRouteServiceEJB.ROUTEALTER);
  ...
  //获得两个版本间的变化。
  private static Collection getPredecessorAlter(TechnicsRouteListIfc iterated1,
                                                TechnicsRouteListIfc iterated2) throws
      QMException {
    PersistService pservice = (PersistService) EJBServiceHelper.
        getPersistService();
    ...
    query.addCondition(cond);
    query.addAND();
    QueryCondition cond1 = new QueryCondition("routeListIterationID",
                                              QueryCondition.GREATER_THAN,
                                              iterateID1);
    query.addCondition(cond1);
    query.addAND();
    QueryCondition cond2 = new QueryCondition("routeListIterationID",
                                              QueryCondition.LESS_THAN,
                                              iterateID2);
    query.addCondition(cond2);
    query.addAND();
    QueryCondition cond3 = new QueryCondition("initialUsed",
                                              QueryCondition.EQUAL, versionID);
    query.addCondition(cond3);
    query.addAND();
    QueryCondition cond4 = new QueryCondition("routeListMasterID",
                                              QueryCondition.EQUAL, masterID);
    query.addCondition(cond4);
    query.addAND();
  ...
  //获得iterated2删除的零件。
  private static Collection getDeleteLink(TechnicsRouteListIfc iterated1,
                                          TechnicsRouteListIfc iterated2) throws
      QMException {
    PersistService pservice = (PersistService) EJBServiceHelper.
        getPersistService();
    ...
    QueryCondition cond = new QueryCondition("alterStatus",
                                             QueryCondition.EQUAL,
                                             TechnicsRouteServiceEJB.PARTDELETE);
    query.addCondition(cond);
    query.addAND();
    QueryCondition cond1 = new QueryCondition("routeListIterationID",
                                              QueryCondition.GREATER_THAN,
    ...
    query.addAND();
    QueryCondition cond2 = new QueryCondition("routeListIterationID",
                                              QueryCondition.LESS_THAN_OR_EQUAL,
                                              iterateID2);
    query.addCondition(cond2);
    query.addAND();
    QueryCondition cond3 = new QueryCondition("initialUsed",
...
///////////////////////////用正常的方式进行版本比较///////
   ...
   * @return Map key = partMasterID, value = Collection:listRoutePartLinkIfc集合。集合顺序，版本号升序排列。
   */
  protected static Map commonCompareIterate(Collection listVec) throws
      QMException {
    Map hashmap = new TreeMap();
    //对集合进行排序。顺序：版本号由小到大。由于bsoid号是升序，按bsoid确定版序。（可排出同一版本的原本和副本的顺序。）
    TechnicsRouteListIfc[] listInfo = RouteHelper.sortedIterate(listVec);
    if (TechnicsRouteServiceEJB.VERBOSE) {
      System.out.println(
          "enter the class:CompareHandler,method:commonCompareIteratie():版本数" +
          listInfo.length);
      for (int i = 0; i < listInfo.length; i++) {
   ...
   * @return Collection
   */
  private static Collection getCommonPartMaster(TechnicsRouteListIfc iterated1,
                                                TechnicsRouteListIfc iterated2,HashMap map) throws
      QMException {
    Collection partVec = new Vector();
    // （ 问题一）性能优化 查看 版本比较算法有冗余 zz start
   ...
   * @throws QMException
   * @return Collection
   * 被修改 author:skybird 2005.01.05
   */
  private static Collection compareCommonIterate(TechnicsRouteListIfc iterated1,
                                                 TechnicsRouteListIfc iterated2,
                                                 Collection partVec,HashMap map) throws
      QMException {
      	//（ 问题一）性能优化 查看 版本比较算法有冗余 zz start
       ...
       // iterated1); 原代码
         vec1 = RouteHelper.getRouteService().getRouteListLinkPartsforVersionCompare(
        iterated1); // end
       ...
       // iterated2); 原代码
 vec2 = RouteHelper.getRouteService().getRouteListLinkPartsforVersionCompare(
 iterated2); //end
    ...
    for (Iterator iter1 = vec1.iterator(); iter1.hasNext(); ) {
      ...
      for (Iterator iter2 = vec2.iterator(); iter2.hasNext(); ) {
        ...
        if (numEql &&
            linkInfo1.getPartMasterID().equals(linkInfo2.getPartMasterID())) {
      ...
      if (flag) {
        ...
        /**
         ...
         ListRoutePartLinkIfc linkInfo1 = (ListRoutePartLinkIfc)iter1.next();
                     ...
                     {
           ListRoutePartLinkIfc linkInfo2 = (ListRoutePartLinkIfc)iter2.next();
           if(linkInfo1.getPartMasterID().equals(linkInfo2.getPartMasterID()))
           {
               if(linkInfo1.getRouteID() == null)
               {
                   if(linkInfo2.getRouteID() != null)
  ...
  private static void compareCommonTwoIterate(TechnicsRouteListIfc iterated1,
                                              TechnicsRouteListIfc iterated2,
                                              Map hashmap) throws QMException {
    Collection vec1 = RouteHelper.getRouteService().getRouteListLinkParts(
        iterated1);
    Collection vec2 = RouteHelper.getRouteService().getRouteListLinkParts(
        iterated2);
    for (Iterator iter1 = vec1.iterator(); iter1.hasNext(); ) {
      ListRoutePartLinkIfc linkInfo1 = (ListRoutePartLinkIfc) iter1.next();
      String number1 = linkInfo1.getParentPartNum();
        ...
        if (numEql &&
          ...
          if (linkInfo1.getRouteID() == null) {
            if (linkInfo2.getRouteID() != null) {
              ...
              hashmap.put(linkInfo1.getPartMasterID() + ";" + number1,
                          linkUpdateVec);
            }
          }
          else if (! (linkInfo1.getRouteID().equals(linkInfo2.getRouteID()))) {
    ...
    for (Iterator iter3 = vec2.iterator(); iter3.hasNext(); ) {
      ...
      hashmap.put(partMasterID + ";" + linkAddInfo.getParentPartNum(),
                  linkAddVec);
    }
  }
