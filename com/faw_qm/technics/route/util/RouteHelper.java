 ...
 * 保留所有权利
 */
package com.faw_qm.technics.route.util;
...
/**
 * 工艺路线服务端类
 * 提供了排序算法,通配符的处理还有输入时间的检验等方法
 * @author 赵立彬
 * @version 1.0
 */
public class RouteHelper {
  private static final String ROUTE_SERVICE = "TechnicsRouteService";
  ...
  public static TechnicsRouteService getRouteService() throws QMException {
    return (TechnicsRouteService) EJBServiceHelper
        .getService(ROUTE_SERVICE);
  }
  ...
  public static String insertKeys(String s, Object aobj[]) {
    if (TechnicsRouteServiceEJB.VERBOSE) {
      System.out.println("RouteHelper's insertKeys ->IN, s = " + s
                         + ", aobj[] = " + aobj[1]);
    }
    ...
    if (TechnicsRouteServiceEJB.VERBOSE) {
      System.out.println("RouteHelper's  insertKeys ->OUT "
                         + stringbuffer);
    }
   ...
   */
  private static Collection sortedInfos(Object[] valueInfos) {
    //排序
...
//          // "+((BaseValueIfc)valueInfos[i+1]).getBsoID());
        ...
        if (! (valueInfos[i] instanceof BaseValueIfc)) {
          throw new QMRuntimeException("此排序只支持BaseValueIfc");
        }
        //由于bsoid号是升序，按bsoid确定版序。（可排出同一版本的原本和副本的顺序。）
        if ( ( (BaseValueIfc) valueInfos[i]).getBsoID().compareTo(
            ( (BaseValueIfc) valueInfos[i + 1]).getBsoID()) > 0) {
          //进行替换
          temp = valueInfos[i];
   ...
   *  存放了一个Object[] ：排序后的BaseValueIfc结果集
   */
  private static Collection sortedInfos(Object[] valueInfos,
   ...
   * @see BaseValueInfo
   */
  private static Comparable getPropertyValue(BaseValueIfc info,
                                             String methodName) {
    Object value = null;
    try {
      ...
      throw new QMRuntimeException(ex);
    }
    if (value == null) {
      throw new QMRuntimeException("要排序的属性值为空。");
    }
    if (! (value instanceof Comparable)) {
      throw new QMRuntimeException("此排序算法暂时只支持Comparable类型");
    }
   ...
   *@return Collection  Object[]集合：<br>
   *指定排序的bsoid,存放（BaseValueIfc）类型元素。
   */
  public static Collection sortedInfos(Set infos) {
   ...
   */
  public static Collection sortedInfos(Collection infos) {
    Object[] valueInfos = infos.toArray();
    return sortedInfos(valueInfos);
  }
   ...
   */
  public static Collection sortedInfos(Collection infos, String methodName,
                                       boolean flag) {
    Object[] valueInfos = infos.toArray();
   ...
   */
  public static TechnicsRouteListIfc[] sortedIterate(Collection listVec) {
    Collection listinfos = sortedInfos(listVec);
    TechnicsRouteListIfc[] routeListInfo = new TechnicsRouteListIfc[listinfos
        .size()];
    int i = 0;
  ...
  public static QueryCondition handleWildcard(String property, String value) throws
      QMException {
  ...
  public static QueryCondition handleWildcard(String property, String value,
    ...
    if (value.indexOf("*") == -1 && value.indexOf("%") == -1) {
      if (flag) {
        ...
        cond = new QueryCondition(property, QueryCondition.NOT_EQUAL,
                                  value);
      }
    }
    else {
      ...
      if (flag) {
        ...
        cond = new QueryCondition(property, QueryCondition.NOT_LIKE,
                                  value);
      }
    }
    return cond;
   ...
   * 设置时间。支持到小时。
   * @param query QMQuery
   * @param time String String 支持格式（精确到小时，可输入时间范围）："2004", "2003-2004", "2003-2004/4",
   *            "2004/04", "2004/4", "2004/4-2004/3" "2004/4/13", "2004/4/13
   *            12", "2004/4 12-2004/4/13 18".

   * @param timeType String  String "createTime" or "modifyTime".
   * @throws QMException
   */
  public static void handleTimeQuery(QMQuery query, String time,
                                     String timeType) throws QMException {
    ...
    if (TechnicsRouteServiceEJB.VERBOSE) {
      System.out
          .println("enter routeService's handleTimeQuery : begin = "
                   + begin);
      //终止时间。
    }
    String end = getValidTime(endString.replace('/', '-').trim(), false);
    if (TechnicsRouteServiceEJB.VERBOSE) {
      System.out.println("enter routeService's handleTimeQuery : end = "
                         + end);
    }
    if (TechnicsRouteServiceEJB.VERBOSE) {
      System.out.println(TechnicsRouteServiceEJB.TIME + "findRouteLists "
                         + timeType + "'s begin = " + begin + ", end= " + end);
    }
    Timestamp beginTime = Timestamp.valueOf(begin);
    Timestamp endTime = Timestamp.valueOf(end);
    QueryCondition cond1 = new QueryCondition(timeType,
                                              QueryCondition.
                                              GREATER_THAN_OR_EQUAL, beginTime);
    query.addCondition(cond1);
    query.addAND();
    QueryCondition cond2 = new QueryCondition(timeType,
