 ...
 * @author 文柳
 */
public final class QMGYBomImportHTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws QMException {
		...
		try {
			...
			// 如果为“导入虚拟件”动作
			if (actionType.equals("importBom")) {
				...
				for(Iterator ite = set.iterator();ite.hasNext();){
					...
					if(!(objs instanceof String)){
						...
						if(file!=null){
							...
							String returnStr = service.importGYBom(byteStr);
...
*/
				...
				request.setAttribute("importBomtLog", logBuffer.toString());
