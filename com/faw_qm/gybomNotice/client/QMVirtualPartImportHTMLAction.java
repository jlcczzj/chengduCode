 ...
 * @author 文柳
 */
public final class QMVirtualPartImportHTMLAction extends HTMLActionSupport {
	...
	public Event perform(HttpServletRequest request) throws QMException {
		...
		try {
			...
			// 如果为“导入虚拟件”动作
			if (actionType.equals("importVirtualPart")) {
				...
				for(Iterator ite = set.iterator();ite.hasNext();){
					...
					if(!(objs instanceof String)){
						...
						if(file!=null){
							...
							Vector vec = service.importVirtualPart(byteStr);
				...
				request.setAttribute("importVirtualPartLog", logBuffer.toString());
